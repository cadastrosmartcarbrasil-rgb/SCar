    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <div className="flex rounded-xl border border-slate-200 bg-superficie p-0.5">
            <button
              onClick={() => trocarVisao('kanban')}
              className={`inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-medium transition ${
                visao === 'kanban' ? 'bg-acao text-white' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
            ...
            <button
              ...
              className={`inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-medium transition ${
                visao === 'lista' ? 'bg-acao text-white' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
          ...
          <Link
            href="/vendas/novo"
            className="inline-flex items-center gap-1.5 rounded-xl bg-acao px-3 py-2 text-sm font-medium text-white shadow-sm"
          >
          ...
          return (
            <button
              ...
              className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-medium transition ${
                active ? 'bg-acao text-white' : 'bg-slate-100 text-slate-600'
              }`}
            >
              {label}
            </button>
          ...
          {(leads ?? []).map((l) => {
            ...
            return (
              <li key={l.id}>
                <Link
                  href={`/vendas/${l.id}`}
                  className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-superficie p-3 active:bg-slate-50"
                >
