...
import type { Combustivel, OrigemFipe } from '@/lib/database.types';
...
export default function NovoLeadPage() {
  ...
  async function gerar() {
    ...
    if (!valorFipe) return toast.error('Informe o valor FIPE');
    try {
      const lead = await salvarLead.mutateAsync({
        nome: nome.trim(),
      ...
      <Secao icon={Calculator} titulo="3. Cotacao">
        ...
        <div>
          ...
          <div className="space-y-1">
            ...
            {opcionais.length === 0 && <span className="text-xs text-slate-400">Nenhum.</span>}
          </div>
        </div>
